...
local goodTags = { "pre", "b", "a", "img", "table", "th", "tr", "td", "!", "br", "tt", "var", "kbd", "code" }

function convert_html(html)
    if html:match(">$") then
        local tag = html:match("</?([!a-z]+)")
        if not has(goodTags, tag) then
            html = html:gsub("<", "&lt;")
        end
    else
        html = html:gsub("<", "&lt;")
...
function format(mark, line, anchor, noSmall)
    local l = ""
    line = line:gsub("\\_", "###LUN###")
    line = line or ""
    line = line:gsub([[<([a-z]+:.-)>]], [[<a href="%1">%1</a>]])
    line = line:gsub("(<[^>]+>?)", convert_html)
    ...
    line = line:gsub("###LUN###", "_")
    if mark and mark:match("^h") and (not noSmall) then
        line = "<small>" .. line .. "</small>"
    end
    if mark then
        l = ("<%s%s>%s</%s>"):format(mark, (anchor and (" id='%s'"):format(anchor) or ""), line, mark, noSmall)
        return l
    else
        return line
    end
...
function makeTOC(tTable)
    ...
    local toc = "<ul style='border: 1px dashed #eee; padding-left: 20px; margin-top: 0px; margin-bottom: 0px;'>"
    local tocLevel = 1
    for k, line in pairs(tTable) do
        if line.level > tocLevel then
            for i = tocLevel+1, line.level do
                toc = toc .. "<ul style='margin-top: 0px; margin-bottom: 0px;'>\n"
            end
        end
        if line.level < tocLevel then
        ...
        tocLevel = line.level
        toc = toc .. ([[<li style='font-size: 15px;'><a href="#%s">%s</a></li>]]):format(line.anchor, line.title)
    end
    for i = 1, tocLevel do
        toc = toc .. "</ul>\n"
    end
...
function markdown(data, noSmall)
    local TOC = {}
    local lines = {}
    local pre = ""
    local inPre = false
    ...
    for line in split(data) do
        ...
        elseif inPre then
            inPre = false
            isKbd = false
            output = table.insert(lines, "<pre>\n" .. ((isKbd and "<kbd>" .. pre:gsub("<", "&lt;") .. "</kbd>") or ("<code>"..pre:gsub("<", "&lt;").."</code>")) .. "</pre><br/>")
            pre = ""
        ...
        if not inPre and not inList then
            ...
            if line:match("^#+[ \t]*([^#{}]+)[ \t]*#*") then
                ...
                table.insert(lines, format("h"..#level, title, anchor, noSmall))
